...
export const reqApi = {
  ...
  submit: (id: number, reviewerId: number) => request({ url: `/requirement/${id}/submit`, method: 'put', params: { reviewerId } }),
  review: (id: number, status: string, reviewerId: number) =>
    request({ url: `/requirement/${id}/review`, method: 'put', params: { status, reviewerId } }),
  ...
  approveChange: (id: number, approved: boolean, comment: string) =>
    request({ url: `/requirement/change/${id}/approve`, method: 'put', params: { approved, comment } }),
